...
describe("User API", () => {
  ...
  describe("DELETE /api/users/logout", () => {
    it("seharusnya berhasil logout jika memberikan token yang valid", async () => {
      ...
      const response = await request(app).delete("/api/users/logout").set("Authorization", "Bearer dummy-token");
    ...
    it("seharusnya gagal logout (401) jika tanpa token", async () => {
      const response = await request(app).delete("/api/users/logout");
  ...
  describe("GET /api/users/current", () => {
    it("seharusnya berhasil mengambil data user yang sedang login", async () => {
      ...
      const response = await request(app).get("/api/users/current").set("Authorization", "Bearer dummy-token");
    ...
    it("seharusnya gagal (401) jika token tidak valid", async () => {
      ...
      const response = await request(app).get("/api/users/current").set("Authorization", "Bearer invalid-token");
  ...
  describe("PATCH /api/users/current", () => {
    it("seharusnya berhasil memperbarui nama", async () => {
      ...
      const response = await request(app).patch("/api/users/current").set("Authorization", "Bearer dummy-token").send({
        name: "Updated Name",
      });
    ...
    it("seharusnya gagal (401) jika tidak terautentikasi", async () => {
      const response = await request(app).patch("/api/users/current").send({
        name: "Ghost Update",
      });
