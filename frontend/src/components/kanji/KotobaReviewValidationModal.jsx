...
/**
 * KotobaReviewValidationModal Component
 * Modal tantangan untuk verifikasi hafalan kosakata sebelum update review count.
 *
 ...
 * @param {object} kotoba - Data kosakata yang sedang direview
 */
export default function KotobaReviewValidationModal({ isOpen, onClose, onSuccess, kotoba }) {
  ...
  useEffect(() => {
    if (isOpen && kotoba) {
      const types = [
        { type: "reading", label: "Reading (Cara Baca)", value: kotoba.reading },
        { type: "meaning", label: "Meaning (Arti)", value: kotoba.meaning },
      ].filter((t) => t.value && t.value.trim() !== ""); // Pastikan field ada isinya

      // Pilih acak satu tipe pertanyaan
      const selected = types[Math.floor(Math.random() * types.length)];
      setQuestion(selected || { type: "meaning", label: "Meaning (Arti)", value: kotoba.meaning });
      setUserAnswer("");
      setStatus("idle");
      setErrorMessage("");
    }
  ...
  const handleSubmit = async (e) => {
    ...
    // Split jawaban benar jika ada koma (misal: "makan, makan malam")
      ...
      <div className="relative bg-background border border-my-border w-full max-w-md rounded-3xl p-6 sm:p-8 shadow-2xl animate-fade-up overflow-hidden">
        ...
        <div className="text-center space-y-6 sm:space-y-8 relative z-10">
          <div className="space-y-2">
            <h3 className="text-xl sm:text-2xl font-black text-white tracking-tight">Vocabulary Challenge</h3>
          ...
          <div className="p-6 sm:p-8 bg-background-lighter/40 rounded-3xl border border-my-border shadow-inner">
            <div className="text-4xl sm:text-5xl font-bold text-primary mb-4 sm:mb-6">{kotoba.word}</div>
            <div className="space-y-1">
              <div className="text-xs font-bold text-secondary-dark uppercase tracking-widest text-center">Question:</div>
              <div className="text-lg text-white font-medium">
                What is the <span className="text-secondary font-bold">{question.label}</span> of this vocabulary?
              </div>
            </div>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="relative">
              <input
                ...
                placeholder={question.type === "reading" ? "e.g. たべる" : "Type your answer here..."}
