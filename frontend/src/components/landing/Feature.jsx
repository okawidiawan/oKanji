  ...
  return (
    <article className={`flex flex-col md:flex-row items-center gap-12 py-16 px-12 ${isReversed ? "md:flex-row-reverse" : ""}`}>
      <div className="w-full md:w-1/2">
        <h3 className={`text-3xl font-bold mb-4 ${isReversed ? "text-center md:text-right" : "text-center md:text-left"}`}>{title}</h3>
        <p className={`text-lg text-secondary-dark leading-relaxed ${isReversed ? "text-center md:text-right" : "text-center md:text-left"}`}>{description}</p>
      </div>
